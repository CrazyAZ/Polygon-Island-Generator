...
public class ElevationGenerator {
	...
	public ElevationGenerator(HashSet<Tile> tiles, int x, int y, int w, int h, IslandGenerator ig, Random rand) {
	...
	private void setPointElevation(Vertex v) {
		double d = Math
				.sqrt((v.x - ig.width / 2) * (v.x - ig.width / 2) + (v.y - ig.height / 2) * (v.y - ig.height / 2))
				/ (ig.height / 2);
